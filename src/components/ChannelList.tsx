      ...
      <div className="flex-1 overflow-y-auto px-2 py-4 space-y-4">
        ...
        <div>
          <div className="flex items-center px-1 mb-1">
            <svg className="w-3 h-3 text-[#949ba4] mr-1" viewBox="0 0 20 20" fill="currentColor">
              <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
            </svg>
            <span className="text-xs font-semibold text-[#949ba4] uppercase tracking-wide">Текстовые каналы</span>
          </div>
          {server.textChannels.map((channel) => (
            <button
              key={channel.id}
              className="w-full flex items-center px-2 py-1.5 rounded text-[#949ba4] hover:text-[#dbdee1] hover:bg-[#35373c] transition-colors group"
            >
        ...
        <div>
          <div className="flex items-center px-1 mb-1">
            ...
            <span className="text-xs font-semibold text-[#949ba4] uppercase tracking-wide">Голосовые каналы</span>
          </div>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${
              isMuted ? 'text-[#ed4245] bg-[#ed4245]/10' : 'text-[#b5bac1] hover:bg-[#35373c]'
            }`}
          ...
          <button
            ...
            className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${
              isDeafened ? 'text-[#ed4245] bg-[#ed4245]/10' : 'text-[#b5bac1] hover:bg-[#35373c]'
            }`}
          ...
          <button
            ...
            className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${
              isStreaming ? 'text-[#ed4245] bg-[#ed4245]/10' : 'text-[#b5bac1] hover:bg-[#35373c]'
            }`}
...
const VoiceChannelItem: React.FC<VoiceChannelItemProps> = ({ channel, isActive, onSelect }) => {
  ...
  return (
    <div>
      <button
        onClick={onSelect}
        className={`w-full flex items-center px-2 py-1.5 rounded transition-colors ${
          isActive ? 'text-white bg-[#404249]' : 'text-[#949ba4] hover:text-[#dbdee1] hover:bg-[#35373c]'
        }`}
      >
        {channel.type === 'stage' ? (
          <svg className="w-4 h-4 mr-1.5 text-[#80848e]" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 3a9 9 0 00-9 9v7c0 1.66 1.34 3 3 3h3v-8H5v-2c0-3.87 3.13-7 7-7s7 3.13 7 7v2h-4v8h3c1.66 0 3-1.34 3-3v-7a9 9 0 00-9-9z"/>
          </svg>
        ) : (
          <svg className="w-4 h-4 mr-1.5 text-[#80848e]" viewBox="0 0 24 24" fill="currentColor">
        ...
        {channel.users.length > 0 && (
          <span className="ml-auto text-xs text-[#949ba4]">{channel.users.length}</span>
        )}
      </button>
